package net.luconia.iconomy.providers;
...
public class VaultEconomy implements Economy {
	private final iConomyRevamped plugin;
	
	public VaultEconomy(iConomyRevamped plugin) {
		this.plugin = plugin;
	}
    ...
    public boolean createPlayerAccount(OfflinePlayer player) {
        ...
        iConomyRevamped.getAccounts().create(player.getUniqueId(), player.getName(), Settings.isNonPlayerAccountName(player.getName()));
        return true;
    }
    ...
    public EconomyResponse depositPlayer(OfflinePlayer player, double amount) {
        ...
        Holdings holdings = iConomyRevamped.getAccounts().get(player.getUniqueId()).getHoldings();
        balance = holdings.balance() + amount;
        holdings.add(amount);
        type = EconomyResponse.ResponseType.SUCCESS;

        iConomyRevamped.getTransactions().insert("[Vault]", player.getName(), 0.0D, balance, 0.0D, amount, 0.0D);
    ...
    public double getBalance(String playerName) {
        ...
        if (player == null)
        	return 0.0;
        return iConomyRevamped.getAccounts().get(playerName).getHoldings().balance();
    }

    @Override
    public double getBalance(OfflinePlayer player) {
        return iConomyRevamped.getAccounts().get(player.getUniqueId()).getHoldings().balance();
    }
    ...
    public boolean hasAccount(String playerName) {
        ...
        return iConomyRevamped.getAccounts().exists(playerName);
    }

    @Override
    public boolean hasAccount(OfflinePlayer player) {

        return iConomyRevamped.getAccounts().exists(player.getUniqueId());
    }
