...
public class VaultUnlockedEconomy implements Economy {
	...
	public VaultUnlockedEconomy(iConomyRevamped plugin) {
		this.plugin = plugin;
	}
	...
	public boolean createAccount(@NotNull UUID accountID, @NotNull String name, boolean player) {
		return iConomyRevamped.getAccounts().create(accountID, name, !player);
	}
	...
	public Map<UUID, String> getUUIDNameMap() {
		return iConomyRevamped.getAccounts().getUUIDNameMap();
	}

	@Override
	public Optional<String> getAccountName(UUID accountID) {
		if (iConomyRevamped.getAccounts().exists(accountID)) {
	...
	public boolean hasAccount(UUID accountID) {
		return iConomyRevamped.getAccounts().exists(accountID);
	}
	...
	public boolean renameAccount(UUID accountID, String name) {
		return iConomyRevamped.getAccounts().get(accountID).setName(name);
	}
	...
	public boolean deleteAccount(String plugin, UUID accountID) {
		iConomyRevamped.getAccounts().get(accountID).remove();
		return true;
	}
	...
	public @NotNull EconomyResponse withdraw(String pluginName, UUID accountID, BigDecimal amount) {
		...
		iConomyRevamped.getTransactions().insert(account.getName(), "[Vault]", 0.0D, account.getHoldings().balance(), 0.0D, 0.0D, amount.doubleValue());
	...
	public @NotNull EconomyResponse deposit(String pluginName, UUID accountID, BigDecimal amount) {
		...
		iConomyRevamped.getTransactions().insert("[Vault]", account.getName(), 0.0D, account.getHoldings().balance(), 0.0D, amount.doubleValue(), 0.0D);
		return new EconomyResponse(amount, getBalance(pluginName, accountID), ResponseType.SUCCESS, null);
	}
